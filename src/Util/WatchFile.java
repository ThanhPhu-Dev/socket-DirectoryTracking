...
public class WatchFile extends Thread{
    ...
    public void filter(String value, String keyword){
        ...
        switch (value){
            case "Thời Điểm":
                setDataTable(folderTrackings.stream().filter(s -> s.getTime().equals(keyword))
                        .collect(Collectors.toList()));
                break;
            case "Action":
                setDataTable(folderTrackings.stream().filter(s -> s.getAction().equals(keyword))
                        .collect(Collectors.toList()));
                break;
        }
    }
